...
function get_sopharmacy_search(input) {
    return `https://sopharmacy.bg/bg/sophSearch/autocomplete/HeaderSearchBoxComponent?term=${input}`;
}

function get_cors_proxy(input) {
    return `https://corsproxy.io/?url=${input}`;
}
...
medicationForm.addEventListener("submit", e => {
	...
	if (currentSearch) {
		const response = fetch(get_cors_proxy(get_sopharmacy_search(currentSearch)), {
			method: "GET",
		});
		response.then(r => {
			return r.json();
			...
			return { medications: obj.products.map(convert) };
		}).then(payload => {
			console.log({payload});
